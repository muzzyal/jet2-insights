...
FlightDataCollection.scrapeCSVData(FlightDataCollection, month.lower(), year, availableMonths, category, dataset, flightDataUrlBase, csvPath)

emailRequest = input("Would you like to receive these files via email? (Y/N)")
emailRequestCheck = True

while emailRequestCheck:
  if emailRequest.upper() == 'Y':
    recipientEmail = input("Please enter your email:")
    sendEmailFunctions.sendEmail(recipientEmail, csvPath, year, category, dataset)
    emailRequestCheck = False
  elif emailRequest.upper() == 'N':
